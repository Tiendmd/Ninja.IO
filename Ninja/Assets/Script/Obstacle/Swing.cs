...
public class Swing : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            child.enabled = true;
            transform.DOMoveX(0, swingTime).SetEase(Ease.Linear);
            c = 1;
            timer = defaultTimer;
        }
        else if (Vector3.Distance(transform.position, new Vector3(0, transform.position.y, transform.position.z)) <= 0.1f && c == 1)
        {
            child.enabled = false;
